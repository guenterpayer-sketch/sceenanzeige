...
- **FRET Songwechsel (gelöst):** Beim Wechsel meldet FRET kurz den neuen Titel mit der alten `startTime` → Balken sprang auf voll (v.a. bei mehreren Playern/Spalten sichtbar). Client-Guard verwirft unplausible Werte (`elapsed > 30s` bei frischem Song), hält den Balken auf 0 und pollt alle 2 s nach bis konsistent (max. 6×). Zusätzlich Event-Nachladen am Songende (Extra-Poll 1,5 s nachdem der Balken 0 erreicht) → neuer Song erscheint sofort statt nach bis zu einem vollen Poll-Intervall. **Kein Instanz-Konflikt:** mehrere FRET-Module (verschiedene Player) laufen unabhängig — jede Instanz eigene Closure/Poll/State, kein globaler Zustand.
