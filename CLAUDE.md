...
- **NC Server-Cache (`includes/NcCache.php`):** Cacht die **rohe** Event-Liste bis Mitternacht in `cache/` (Fallback `sys_get_temp_dir()`, wenn nicht beschreibbar; `.htaccess` sperrt den Ordner). Schlüssel = **nur** die API-Parameter (`date_days`), **nicht** Standort-/Saal-Filter — die filtert `nc.php` erst danach über die gecachten Daten. Dadurch teilen sich alle Säle und alle Modul-Instanzen **einen** Abruf pro Tag. Invalidierung ausschließlich über `admin/reload_trigger.php` („Monitore neu laden" → `NcCache::leeren()`). **Bewusst KEIN `?frisch=1`-Parameter:** `proxies/nc.php` ist ohne Login erreichbar, ein Cache-Bypass von außen könnte das Kontingent leerrufen. `nc.php` liefert zusätzlich die Diagnosefelder `quelle` (`api`/`cache`) und `stand` — das Frontend wertet sie nicht aus.
