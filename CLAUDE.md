...
| 29 | Kalender-Planungstool in Etappen — **A:** Tabelle `monitor_termine` + Migration 14 + zwei-Ebenen-Auswahl in `Monitor.php` + Nav „Kalender" + Kalender-Tab als Standard im Zeitplan-Editor; **B:** `wochenplan.php` → echter Kalender mit Datum (← Heute →, Regelbetrieb dezent + Termine kräftig), nur lesen; **C:** Termin-Dialog (anlegen/bearbeiten/löschen/duplizieren, Monitor-Checkboxen, Sofort-Speichern per AJAX); **D:** Politur (Drag/Resize, Termine lesend im Monitor-Editor) | A ⏳ Staging, zu testen · B–D geplant |
